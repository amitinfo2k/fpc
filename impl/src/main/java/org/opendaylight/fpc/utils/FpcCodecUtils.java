...
public class FpcCodecUtils {
   ...
   /**
     ...
     */
    public ConfigureInput ConfigureInputFromJsonString(String inputJson) {
        ...
        JsonParserStream jsonParser = JsonParserStream.create(streamWriter, context, CONFIGURE);
        try{
        	jsonParser.parse(new JsonReader(new StringReader(inputJson)));
        } catch(Exception e){
        	result = new NormalizedNodeResult();
            streamWriter = ImmutableNormalizedNodeStreamWriter.from(result);
            jsonParser = JsonParserStream.create(streamWriter, context, CONFIGURE);
            NormalizedNode<?, ?> transformedInput = result.getResult();
            try{
            	return (ConfigureInput) codecRegistry.fromNormalizedNodeRpcData(CONFIGURE_INPUT_PATH, (ContainerNode) transformedInput);
            }
            catch(Exception e1){
